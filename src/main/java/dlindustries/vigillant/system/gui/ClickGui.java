...
public final class ClickGui extends Screen {
	...
	public ClickGui() {
		...
		for (Category category : Category.values()) {
			boolean compact = category == Category.DONUT;
			int width = compact ? 168 : 230;
			int height = compact ? 14 : 30;
			int y = compact ? 200 : 50;
			Window window = new Window(offsetX, y, width, height, category, this, compact);
			if (compact) window.extended = false;
			windows.add(window);
			offsetX += compact ? 178 : 250;
		}
	}
	...
	private void renderPlayerName(DrawContext context) {
		...
		String systemText = "Regen Client{BETA}";
	...
	public void onGuiClose() {
		mc.execute(() -> mc.setScreen(system.INSTANCE.previousScreen));
		currentColor = null;
		for (Window window : windows)
			window.onGuiClose();
